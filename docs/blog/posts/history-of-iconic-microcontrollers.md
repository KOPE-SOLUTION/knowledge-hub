...
tags:
  ...
  - ESP32
  - RISC-V
summary: "ย้อนดูวิวัฒนาการ MCU ผ่าน 16 ตระกูลสำคัญ พร้อม ESP8266 และผู้ผลิตรายอื่น ตั้งแต่ชิป 4-bit ไปสู่ Arm, Wireless SoC, dual-core, RISC-V และ MCU สำหรับงานความปลอดภัยสูง"
...
บทความนี้เลือก MCU 16 ตระกูลหรือรุ่นที่มีบทบาทสำคัญต่อวิวัฒนาการของวงการ และเพิ่ม ESP8266 เป็นหัวข้อเสริมก่อนเข้าสู่ยุค ESP32 เนื้อหาครอบคลุมตั้งแต่การรวม CPU, memory และ I/O ไว้ในชิปเดียว ไปจนถึง Flash ที่เขียนซ้ำได้, Arm 32-bit, Bluetooth LE, Wi-Fi และ MCU แบบ dual-core

หมายเลข 1–16 เรียงตามช่วงเวลา ไม่ใช่อันดับความดีที่สุดหรือยอดขาย โดยเลือกแต่ละรุ่นเพื่อแสดงเทคโนโลยีที่โดดเด่นในช่วงเวลานั้น

## สรุปเส้นเวลา

| ช่วงเวลา | MCU หรือตระกูล | สถาปัตยกรรม | จุดเด่นและความสำคัญ |
|---|---|---|---|
...
| 2007 | STM32F103 | 32-bit Arm Cortex-M3 | Cortex-M พร้อมระบบเครื่องมือ ซอฟต์แวร์ และชุมชนสำหรับ MCU 32-bit |
...
| 2013–2014 | Espressif ESP8266 (หัวข้อเสริม) | 32-bit Tensilica L106 | ทำให้ Wi-Fi SoC ราคาประหยัดเข้าถึง Maker และงาน IoT ก่อนยุค ESP32 |
...
| 2022 | Nations N32G430 / N32 family | 32-bit Arm Cortex-M4F | Cortex-M4F 128 MHz พร้อม FPU, DSP และวงจรต่อพ่วงสำหรับงานควบคุม |

ปีในตารางมีทั้ง “ปีเปิดตัวตระกูล” และ “ช่วงที่รุ่นตัวอย่างได้รับความนิยม” โดยรายละเอียดของแต่ละหัวข้อจะระบุบริบทเพิ่มเติม ส่วน ESP8266 เป็นหัวข้อเสริมระหว่างลำดับที่ 13 และ 14 จึงไม่มีหมายเลขแยกในลำดับ 1–16

## ก่อนเปรียบเทียบ: MHz ไม่ใช่คำตอบทั้งหมด

ตัวเลข clock บอกจำนวนจังหวะต่อวินาที แต่ไม่ได้บอกว่า MCU ทำงานได้เท่าไรต่อหนึ่งจังหวะ สถาปัตยกรรมแต่ละแบบอาจใช้จำนวน clock ต่อคำสั่งไม่เท่ากัน และยังมีความแตกต่างด้านโครงสร้าง CPU, ความเร็วหน่วยความจำ, bus, cache และฮาร์ดแวร์ช่วยประมวลผล

ตัวอย่างความแตกต่างของระบบ clock มีดังนี้
...
- MCU รุ่นใหม่อาจให้ผลลัพธ์มากกว่าต่อหนึ่ง clock และมี DMA หรือวงจรต่อพ่วงช่วยทำงานแทน CPU

การเปรียบเทียบที่มีความหมายจึงต้องใช้งานทดสอบแบบเดียวกัน และพิจารณาเวลาในการตอบสนอง การใช้พลังงาน ขนาดหน่วยความจำ วงจรต่อพ่วง และเครื่องมือพัฒนาร่วมด้วย
...
ประวัติ “MCU ตัวแรก” มีรายละเอียดเรื่องวันที่ประดิษฐ์ วันที่จดสิทธิบัตร วันที่ประกาศ และวันที่เริ่มจำหน่ายต่างกัน ดังนั้นคำว่า “ยุคแรก” จึงแม่นยำกว่าการตัดสินด้วยปีเพียงตัวเดียว
...
ก่อน Z8 และ 8051 จะเปิดตัวในช่วงปี 1979–1980 ตลาดมี MCU ที่ถูกนำไปใช้ในผลิตภัณฑ์จริงแล้ว โดย MCS-48 เป็นหนึ่งในตัวแทนสำคัญของช่วงเวลาดังกล่าว
...
Z8 ถูกนำไปใช้ในอุปกรณ์ผู้บริโภค ระบบรักษาความปลอดภัย HVAC และงานควบคุมหลากหลายประเภท จุดเด่นของสถาปัตยกรรมคือ register file ซึ่งแบ่งเป็นกลุ่ม working registers และใช้ Register Pointer เลือกกลุ่มที่กำลังทำงาน แนวคิดนี้ช่วยให้คำสั่งบางรูปแบบอ้างรีจิสเตอร์ได้กระชับ และช่วยสลับบริบทได้รวดเร็วขึ้น

ชิปที่เขียนว่า `Z8S18020` เป็นสมาชิกของสาย Z180 ไม่ใช่ Z8 รุ่นดั้งเดิม ดังนั้นตัวเลข 20 MHz ของชิปดังกล่าวจึงไม่ใช่คุณสมบัติของ Z8 ที่เปิดตัวในปี 1979
...
สิ่งที่ทำให้ 8051 มีอิทธิพลคือความยืดหยุ่น รวมถึงระบบเครื่องมือ ซอฟต์แวร์ และผู้พัฒนาที่แพร่หลาย Intel ระบุว่า MCS-51 ขายได้ 100 ล้านชิ้นภายในทศวรรษแรก และถูกใช้ตั้งแต่ระบบเบรก เครื่องบิน เครื่องดนตรี ไปจนถึงของเล่น หลังจากนั้นผู้ผลิตรายอื่นยังคงสร้างชิปที่ใช้สถาปัตยกรรมหรือชุดคำสั่งเข้ากันได้ต่อมาอีกยาวนาน
...
การเปรียบเทียบ 68HC11 กับ MCU รุ่นใหม่จึงต้องแยกความถี่คริสตัลออกจาก E-clock และพิจารณาหมายเลขรุ่นย่อยร่วมด้วย
...
การแยก “Atmel AVR รุ่นแรก” ออกจาก “ATmega328P บน Arduino” จึงสำคัญ เพราะเหตุการณ์แรกเป็นเรื่องของสถาปัตยกรรม ส่วนเหตุการณ์หลังแสดงอิทธิพลของเครื่องมือ ซอฟต์แวร์ และชุมชน โดยทั้งสองเหตุการณ์เกิดขึ้นคนละช่วงเวลา
...
LPC2148 เริ่มต้นในยุค Philips Semiconductors ขณะที่ NXP ก่อตั้งขึ้นจากการแยกธุรกิจเซมิคอนดักเตอร์ของ Philips ในปี 2006 ชื่อทางประวัติศาสตร์ของรุ่นนี้จึงเป็น Philips LPC2148 ส่วนปัจจุบันข้อมูลผลิตภัณฑ์อยู่ภายใต้ NXP
...
## 10. STM32F103 — Cortex-M3 และระบบเครื่องมือสำหรับ MCU 32-bit
...
![ภาพรวม STM32F103 แสดงบอร์ดควบคุมหุ่นยนต์และมอเตอร์ ควบคู่กับ NVIC, DMA และบัสวงจรต่อพ่วง](../../assets/images/blog/history-of-iconic-microcontrollers/10-stm32f103-overview.jpg)
...
คำสั่งอย่าง `pinMode()` และ `digitalWrite()` ลดกำแพงในการเริ่มต้น ผู้เรียนสามารถทำให้ LED ติดก่อน แล้วจึงค่อยย้อนกลับไปศึกษา register, timer และ interrupt ภายหลัง ATmega328P จึงเป็นตัวอย่างที่ชัดเจนว่าเครื่องมือ ซอฟต์แวร์ และชุมชนสามารถทำให้ MCU หนึ่งรุ่นมีอิทธิพลเกินกว่าสเปกบน datasheet ได้
...
![ภาพรวม Renesas RL78 แสดงแผงควบคุมเครื่องใช้ไฟฟ้า มิเตอร์ และสายการผลิต ควบคู่กับการใช้พลังงานต่ำและการรวมวงจรต่อพ่วง](../../assets/images/blog/history-of-iconic-microcontrollers/12-renesas-rl78-overview.jpg)


RL78 เป็นตัวอย่างที่ดีว่าโลก embedded ไม่ได้เปลี่ยนจาก 8-bit ไป 32-bit พร้อมกันทั้งหมด งานที่ผลิตจำนวนมากยังให้ความสำคัญกับกระแสขณะพัก เวลาในการตื่นกลับมาทำงาน วงจรต่อพ่วงที่ตรงงาน อายุการจัดจำหน่าย และต้นทุนรวมมากกว่าคะแนนประมวลผลสูงสุด

Renesas ยังสะท้อนอีกด้านของอุตสาหกรรมที่ต่างจากบอร์ด Maker เพราะ MCU จำนวนมหาศาลทำงานอยู่ในผลิตภัณฑ์ที่ผู้ใช้ไม่เคยเห็นบอร์ดหรือชื่อชิป

## 13. Nordic nRF51 / nRF52 — Bluetooth LE กลายเป็นส่วนหนึ่งของ MCU

Nordic Semiconductor เปิดตัว nRF51 แบบ single-chip Bluetooth Low Energy ในปี 2012 โดยรวมวิทยุ 2.4 GHz เข้ากับ Arm Cortex-M0 และหน่วยความจำใน SoC เดียว ต่อมา nRF52832 ซึ่งเปิดตัวในปี 2015 ใช้ Cortex-M4F และเพิ่มสมรรถนะ หน่วยความจำ และวงจรต่อพ่วงสำหรับอุปกรณ์สวมใส่ เซนเซอร์ไร้สาย อุปกรณ์สุขภาพ และอุปกรณ์เชื่อมต่อพลังงานต่ำ
...
จุดเด่นของ Nordic ไม่ใช่เพียงคำว่า “32-bit” แต่คือการรวมภาครับส่งวิทยุ ซอฟต์แวร์โปรโตคอล และ MCU สำหรับรันแอปพลิเคชันไว้ในชิปเดียว นักพัฒนาจึงสร้างอุปกรณ์ Bluetooth LE ได้โดยไม่ต้องจับคู่ MCU หลักกับโมดูลวิทยุแยกกันเสมอไป
...
## ESP8266 (2013–2014) — Wi-Fi ราคาประหยัดเข้าสู่งาน IoT

ESP8266 เป็นสะพานสำคัญก่อนถึง ESP32 เพราะทำให้ Wi-Fi สำหรับงาน IoT มีต้นทุนและจำนวนชิ้นส่วนลดลงอย่างมาก Espressif ประกาศการผลิต ESP8266 ปริมาณมากในเดือนธันวาคม 2013 ขณะที่หน้า milestones ระบุเดือนพฤษภาคม 2014 เป็นช่วงเปิดตัว ESP8266EX ในฐานะ IoT SoC ตัวแรกของบริษัท ความต่างนี้สะท้อนว่าคำว่า “ปีเปิดตัว” อาจหมายถึงการประกาศผลิต การวางตลาด หรือการทำตลาดในวงกว้าง
...
หลัง ESP8266 เปิดตลาด Wi-Fi SoC ราคาประหยัดแล้ว ESP32 รุ่นดั้งเดิมเปิดตัวในปี 2016 และขยายแนวคิดไปอีกขั้นด้วย Xtensa LX6 แบบ 32-bit มีทั้งรุ่น single-core และ dual-core โดยรองรับความถี่สูงสุด 240 MHz พร้อม Wi-Fi, Bluetooth Classic, Bluetooth Low Energy, SRAM, ADC, DAC, วงจรตรวจจับการสัมผัส และวงจรต่อพ่วงสำหรับงานควบคุมจำนวนมาก
...
## 16. Nations N32 — ผู้ผลิตรายใหม่และทางเลือกด้านการจัดหา

Nations Technologies มีตระกูล N32 ที่ใช้แกน Arm Cortex-M หลายระดับ ครอบคลุมงานควบคุมทั่วไป มอเตอร์ วงจรสัญญาณแอนะล็อกและดิจิทัล รวมถึงงานที่ต้องการคุณสมบัติด้านความปลอดภัย รุ่น N32G430 ซึ่งประกาศในปี 2022 ใช้ Cortex-M4F ทำงานได้สูงสุด 128 MHz และรวม FPU, DSP กับวงจรต่อพ่วงสำหรับงานควบคุม
...
แม้ N32 จะไม่ได้มีประวัติยาวนานเท่า 8051 หรือ AVR แต่ตระกูลนี้สะท้อนความเปลี่ยนแปลงของอุตสาหกรรมปัจจุบัน นักพัฒนาไม่ได้เลือกชิปจากผู้ผลิตตะวันตกหรือญี่ปุ่นเพียงไม่กี่รายอีกต่อไป ปัจจัยด้านความพร้อมของสินค้า เครื่องมือ เอกสาร การสนับสนุนจากชุมชน และความเข้ากันได้ของวงจรต่อพ่วงจึงมีน้ำหนักมากขึ้น

N32 ยังแสดงให้เห็นว่าตลาด MCU ปัจจุบันมีผู้ผลิตให้เลือกมากขึ้น นอกเหนือจากบริษัทที่ครองตลาดมายาวนาน ยังมี GigaDevice GD32, WCH CH32, Bouffalo Lab และ MCU ที่ใช้ RISC-V ความหลากหลายนี้ทำให้นักพัฒนาต้องพิจารณาทั้งราคา ความพร้อมของสินค้า เครื่องมือ เอกสาร และการสนับสนุนระยะยาวร่วมกัน

## ตระกูล MCU สำคัญอื่น ๆ

นอกจากรุ่นที่อธิบายโดยละเอียดด้านบน ยังมี MCU อีกหลายตระกูลที่มีบทบาทสำคัญในเครื่องใช้ไฟฟ้า ยานยนต์ อุตสาหกรรม ระบบพลังงานต่ำ ระบบอนาล็อกที่ปรับแต่งได้ และการสื่อสารไร้สาย

![แผนที่ภาพรวมวิวัฒนาการ MCU หลายเส้นทาง ตั้งแต่งานควบคุม 8-bit ไปสู่ระบบแอนะล็อก ระบบเครื่องมือ Arm ความปลอดภัยยานยนต์ ระบบไร้สาย และสถาปัตยกรรมเปิด](../../assets/images/blog/history-of-iconic-microcontrollers/parallel-mcu-families-map-overview.jpg)

| ช่วงเวลาโดยประมาณ | ตระกูลหรือค่าย | จุดเด่นและบทบาทสำคัญ |
|---|---|---|
| ปลายทศวรรษ 1980 ถึงทศวรรษ 1990 | Motorola 6805 / 68HC05 / 68HC08 | ใช้อย่างแพร่หลายในเครื่องใช้ไฟฟ้า ยานยนต์ และงานควบคุม 8-bit ต้นทุนต่ำ ควบคู่กับ 68HC11 |
| ต้นทศวรรษ 2000 | Cypress PSoC 1 ซึ่งปัจจุบันอยู่กับ Infineon | รวม MCU กับบล็อก analog และ digital ที่กำหนดค่าได้ ทำให้ปรับวงจรต่อพ่วงให้เหมาะกับงานได้มากขึ้น |
| 2009 เป็นต้นมา | Renesas RX | ใช้ CPU core 32-bit ของ Renesas เอง โดยเน้นประสิทธิภาพ ความกระชับของโปรแกรม และการควบคุมที่ตอบสนองทันที เป็นอีกทางเลือกนอกเหนือจาก Arm |
| เริ่มช่วง 2010–2011 | Freescale/NXP Kinetis | ช่วยขยาย Cortex-M4 ไปสู่งานทั่วไป เครื่องมือวัด และอุตสาหกรรม พร้อมเครื่องมือและบอร์ดพัฒนาของผู้ผลิต |
| 2012 เป็นต้นมา | Infineon AURIX / TriCore | เน้นงานยานยนต์ด้วยการตอบสนองแบบ real-time, CPU หลายแกน, functional safety, การทำงานสำรองแบบ lockstep และระบบความปลอดภัยในฮาร์ดแวร์ |
| 2013 (Zero Gecko) และตระกูลต่อมา | Silicon Labs EFM32 Gecko | นำแนวคิด MCU พลังงานต่ำมาสู่ Cortex-M ด้วยโหมดพลังงานหลายระดับและวงจรต่อพ่วงที่ทำงานได้โดยไม่ต้องปลุก CPU ตลอดเวลา |
| กลางทศวรรษ 2010 | Atmel/Microchip SAM D21 | นำ Cortex-M0+ พลังงานต่ำ พร้อม USB และวงจรแอนะล็อก ไปใช้ในบอร์ดพัฒนาและผลิตภัณฑ์ขนาดเล็กจำนวนมาก |
...
| 2013–2014 | Espressif ESP8266 | ทำให้ Wi-Fi SoC ราคาประหยัดเข้าถึง Maker และงาน IoT ก่อน ESP32 ด้วยแรงสนับสนุนจากโมดูล ซอฟต์แวร์ และชุมชน |
| 2013 เป็นต้นมา | GigaDevice GD32 | เพิ่มตัวเลือก MCU ทั้ง Arm Cortex-M และ RISC-V พร้อมการแข่งขันด้านราคาและความพร้อมของสินค้า |
| ปลายทศวรรษ 2010 เป็นต้นมา | NXP i.MX RT | ลดช่องว่างระหว่าง MCU กับหน่วยประมวลผลแอปพลิเคชัน ด้วย Cortex-M ความถี่สูง หน่วยความจำขนาดใหญ่ และช่องเชื่อมต่อสำหรับภาพ เสียง Ethernet และงานที่ต้องตอบสนองทันที |
| ต้นทศวรรษ 2020 | WCH CH32V | ทำให้ MCU RISC-V ราคาต่ำเข้าถึงผู้เรียนและงานควบคุมทั่วไปได้ง่ายขึ้น |
| ปลายทศวรรษ 2010 ถึงต้นทศวรรษ 2020 | Bouffalo Lab BL602 / BL604 | เพิ่มทางเลือก wireless SoC ที่รวม RISC-V, Wi-Fi และ Bluetooth LE นอกเหนือจาก Espressif และ Nordic |

รายชื่อนี้ยังไม่ครอบคลุม MCU ทุกตระกูล แต่ช่วยให้เห็นว่าตลาดพัฒนาไปพร้อมกันหลายด้าน ทั้งยานยนต์ ระบบไร้สาย RISC-V ระบบอนาล็อกที่กำหนดค่าได้ และระบบเครื่องมือกับซอฟต์แวร์ของ Cortex-M
...
3. **วงจรต่อพ่วง:** timer, ADC, serial, USB, CAN, DMA และ programmable I/O เพิ่มขึ้น
4. **พลังงาน:** มีการปิดสัญญาณ clock ของส่วนที่ไม่ได้ใช้, sleep modes และระบบปลุกกลับมาทำงานที่ละเอียดขึ้น
...
7. **ความปลอดภัย:** secure boot, วงจรช่วยเข้ารหัส และการปกป้องกุญแจดิจิทัลกลายเป็นข้อกำหนดสำคัญ
8. **การจัดหา:** อายุผลิตภัณฑ์ แหล่งจัดซื้อ เอกสาร และผู้ผลิตทางเลือกมีผลต่อการออกแบบตั้งแต่ต้น
